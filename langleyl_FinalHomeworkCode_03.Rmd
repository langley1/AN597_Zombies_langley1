---
title: "ishratc_PeerCommentary_langley1_03"
...
# Peer Commentary by Ishrat (Now with 2nd LA Comments)

1. I had a lot of trouble with this one and didn't get as far as you did but here is my input. It, does look like you pulled out the samples correctly. I liked how you did summary, that makes it really clear that they were pulled out correctly. I would expect different results for your mean as well, but I looked a litle firther into what you were using to calculate the mean and it looks like the mean value you are gettin is for 1000 samples. I found this out using the code below.No matter what you put in you get 1000 of the same samples. I'm wondering if you need to reselect 99 samples form this.I'm not sure if that really helps.

LA: Oh you're so right, it returns 1000 results. That's strange since I only used 99 as my k for # of samples.. but when I so summary(s2) it shows that there are only 99 rows, which is why I thought there were 99 samples
...
s3 <- NULL # dummy variable to hold each sample
for (i in 1:k){
    s3[[i]] <- sample(s2,size=n, replace=TRUE) #I think we have to set replace to TRUE because     30 x 99= 2970 and "zombies" df only has 1000 rows
}
summary(s3)

s3[[1]]$weight
#Ok so I tried to pull out 99 samples from s2 but it now returns a NULL when I try to look at just the weight variable, so this isn't a solution
```

I also did something similar and found that the age category (and perhaps all the others) was null. I'm wondering if you need to change the type of information it is. I tried doing it myself below, but was unsuccessful.

```{r}
str(s2[[1]]$weight)
str(s2[[1]]$age)
(s2[[1]]$age)
(s2[[1]]$age) <- as.numeric((s2[[1]]$age))

#Hmm, yes this goes along with my issue up above. The varibale returns a NULL. great idea to try to change it, I still can't figure out why it would be coming back as NULL as well. Definitley still need some more guidance on this one.
```

For combining it with the old sample, I would have also tried concatonating as you did, but I think it is beign werid because it sin't matching up exactly. I'm assuming this the same reason as why the previous codes aren't working as well.

2. I think you did it correclty, I had trouble understading this as well, but looking at your code, this makes sense.

3. I don't think you can knit with the install.packages code line. I think you would have to take this out of the R code. 

LA: yes, perhaps I can't have it in my chunks

4. Everything else looks really good, I think it is well annotated. Great job!

LA: Thanks!

Sorry I wasn't much help but it looks like you got most of it!
